...
public class RunwayLights : MonoBehaviour
{
    ...
    public Transform runwayPosition, taxiwayPosition1;
    ...
    private class RunwayData
    {
        ...
        public void SetThresholdPosition(Vector3 position, float lengthIn)
        {
            ...
            //print(thresholdPosition[0] + " " + thresholdPosition[1]);
        }
        ...
        public bool taxiwayLights;
        public float taxiwayLightsSpacing;
    }
    ...
    public void CreateRunwayLights()
    {
        ...
        RunwayData[] runways = new RunwayData[1];
        ...
        runways[index].SetThresholdPosition(new Vector3(0, 0, 0), 2000);
        runways[index].thresholdHeight[0] = 0;
        runways[index].thresholdHeight[1] = 0;
        runways[index].midHeight = .1f;
        runways[index].SetThresholdRotation(0);
        runways[index].width = 40;
        runways[index].lightingType[0] = LightingType.NONE;
        runways[index].lightingType[1] = LightingType.NONE;
        runways[index].strobeType[0] = StrobeType.NONE;//ODALS;
        runways[index].strobeType[1] = StrobeType.NONE;//BOTH;
        runways[index].papiType[1] = PapiType.NONE;//PAPIBOTH;
        runways[index].papiType[0] = PapiType.NONE;//PAPIBOTH;
        ...
        runways[index].taxiwayLights = true;

        /*
        index = 1;
        runways[index] = new RunwayData();
        runways[index].SetThresholdPosition(new Vector3(0, 0, 0), 300); //print(taxiwayPosition1.position);
        runways[index].thresholdHeight[0] = 0;
        runways[index].thresholdHeight[1] = 0;
        runways[index].midHeight = 0;
        runways[index].SetThresholdRotation(90);
        runways[index].width = 45;
        runways[index].lightingType[0] = LightingType.NONE;
        runways[index].lightingType[1] = LightingType.NONE;
        runways[index].strobeType[0] = StrobeType.NONE;
        runways[index].strobeType[1] = StrobeType.NONE;
        runways[index].papiType[0] = PapiType.NONE;
        runways[index].papiType[1] = PapiType.NONE;
        runways[index].centerlineLights = false;
        runways[index].centerlineLightsSpacing = 15;
        runways[index].edgeLights = false;
        runways[index].edgeLightsSpacing = 30;
        runways[index].thresholdWingbar[0] = ThresholdWingbar.LARGE;
        runways[index].thresholdWingbar[1] = ThresholdWingbar.LARGE;
        runways[index].TDZ[0] = false;
        runways[index].TDZ[1] = false;
        ...
        index = 2;
        runways[index] = new RunwayData();
        runways[index].SetThresholdPosition(new Vector3(20, 0, 1220), 300);
        runways[index].thresholdHeight[0] = 0;
        runways[index].thresholdHeight[1] = 0;
        runways[index].midHeight = 0;
        runways[index].SetThresholdRotation(30);
        runways[index].width = 20;
        runways[index].lightingType[0] = LightingType.NONE;
        runways[index].lightingType[1] = LightingType.NONE;
        runways[index].strobeType[0] = StrobeType.NONE;
        runways[index].strobeType[1] = StrobeType.NONE;
        ...
        runways[index].centerlineLightsSpacing = 30;
        runways[index].edgeLights = false;
        runways[index].edgeLightsSpacing = 30;
        runways[index].thresholdWingbar[0] = ThresholdWingbar.NONE;
        runways[index].thresholdWingbar[1] = ThresholdWingbar.NONE;
        runways[index].TDZ[0] = false;
        runways[index].TDZ[1] = false;

        index = 3;
        runways[index] = new RunwayData();
        runways[index].SetThresholdPosition(new Vector3(20, 0, 1265), 300);
        runways[index].thresholdHeight[0] = 0;
        runways[index].thresholdHeight[1] = 0;
        runways[index].midHeight = 0;
        runways[index].SetThresholdRotation(30);
        ...
        index = 4;
        runways[index] = new RunwayData();
        runways[index].SetThresholdPosition(new Vector3(40, 0, 530), 170);
        runways[index].thresholdHeight[0] = 0;
        runways[index].thresholdHeight[1] = 0;
        runways[index].midHeight = 0;
        runways[index].SetThresholdRotation(80);
        runways[index].width = 40;
        runways[index].lightingType[0] = LightingType.NONE;
        runways[index].lightingType[1] = LightingType.NONE;
        runways[index].strobeType[0] = StrobeType.NONE;
		...
		*/
        ...
        for (int i = 0; i < runways.Length; i++)
        {
            ...
            SetupThresholdLights(ref directionalLightData, runways[i], randomBrightness, 0.6f, largerLightSize * scale);
            SetupCenterlineLights(ref omnidirectionalLightData, runways[i], randomBrightness, brightness, smallerLightSize * scale);
            ...
            SetupTaxiWayLights(ref omnidirectionalLightData, runways[i], randomBrightness, brightness, scale);
        }
        ...
        //parentObject.transform.rotation = Quaternion.Euler(0, 0, 0);
        //parentObject.transform.position = new Vector3(0, 0, 0);
        parentObject.transform.position = runwayPosition.position;
        parentObject.transform.rotation = runwayPosition.rotation;
        ...
        if (runwayData.centerlineLights)
        {
            ...
            for (int i = 0; i < lightAmountInRow; i++)
            {
                ...
                if (currentDistance <= redA)
                {
                    ...
                    data.frontColor = whiteLight;
                    data.brightness = SetBrightness(randomBrightness, brightness);
                    data.backColor = whiteLight;//redLight;
                ...
                if (currentDistance >= redB)
                {
                    ...
                    data.frontColor = whiteLight;//redLight;
                    data.brightness = SetBrightness(randomBrightness, brightness);
    ...
    //Here i'm trying to instantiate a single light in the position of the gameobject taxiwaypoint1 passed on as a public Transform
    private void SetupTaxiWayLights(ref List<SpriteLights.LightData> lightData, RunwayData runwayData, bool randomBrightness, float brightness, float size)
    {

        if (runwayData.taxiwayLights)
        {
            //float sideFactor = 0;

            //Transform parent = GetComponentInParent<Transform>();

            //int lightAmountInRow = (int)Mathf.Floor(runwayData.length / runwayData.taxiwayLightsSpacing);
            ...
            data.position = taxiwayPosition1.position;
            data.frontColor = greenLight;
            data.brightness = SetBrightness(randomBrightness, brightness);
            data.backColor = greenLight;
            data.size = size;
            lightData.Add(data);
